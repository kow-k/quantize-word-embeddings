...
When testing the save/load pipeline:
...
class TestSaveLoad:
    ...
    def test_save_load_roundtrip(self):
        """Test that save/load preserves embeddings."""
        ...
        try:
            # Save
            save_embeddings(embeddings, words, temp_path)
            
            # Load
            loaded_emb, loaded_words = load_embeddings(temp_path)
            
            # Verify
            assert loaded_emb.shape == embeddings.shape
            assert loaded_words == words
            np.testing.assert_array_almost_equal(loaded_emb, embeddings, decimal=5)
            
        finally:
            os.unlink(temp_path)
    
    def test_quantized_save_load(self):
        """Test saving quantized embeddings."""
        embeddings = np.random.randn(100, 50)
        words = [f'word{i}' for i in range(100)]
        ...
        try:
            # Save with metadata
            save_embeddings(quantized, words, temp_path,
            ...
            assert loaded_emb.shape == quantized.shape
            assert loaded_words == words
